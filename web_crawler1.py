	...
	with open('./link_list.json') as link_json:
		...
		for i in range(1, int(link_list[-1])):
			for j in range(x, x+10):
				before_urls.append(link[str(i)][str(j)]) 
				# URL 에서 숫자만 추출하기 
				# number = re.findall("\d+" , before_urls)[0]
...
def access_page(enter_url):
		req_page = requests.get("http://helpline.nih.go.kr/cdchelp/" + enter_url)
...
def make_dictionary(name , contents, content_json):
		# dictionary 만들기. 	
		for content in contents:
			#modified_name = (''.join(name.split())).replace("/", " ")
			#text = content.text
...
if __name__ == '__main__':
	...
	with open("./1.json", 'w', encoding = 'UTF8') as content_json:
		pool.map(access_page, get_code_no());
		#pool.map(access_page, before_urls);
